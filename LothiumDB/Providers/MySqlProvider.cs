...
public class MySqlProvider : BaseProvider, IProvider
{
    ...
    public IDbCommand CreateCommand(string command, IDbConnection connection, IDbTransaction? transaction)
    {
        ...
        return ((MySqlTransaction)transaction! is null)
            ? new MySqlCommand(command, (MySqlConnection)connection)
            : new MySqlCommand(command, (MySqlConnection)connection, (MySqlTransaction)transaction);
    }
